...
% - Incorporate remaining Zhao comments
...
% - SOLEDAD: address constraints may not be satisfiable
...
Since it is an overloaded concept, we pause here to make a few comments about ``over-fitting''.... HOGG
...
HOGG: INSERT POINT ABOUT OVER-CHOOSING YOUR HYPER-PARAMETERS: Over-fitting to the validation operator.
...
\begin{equation} \label{qp}
    \arg\min_x \frac{1}{2} x^T G x + x^\top c \quad \text{ subject to } Ax=b.
\end{equation}
...
Using this formulation it is easy to check that the solution of \eqref{eq:opt2} leads to predictions in \eqref{eq:ridge_sol}, and the solution of \eqref{eq:weighted} produce \eqref{eq:weighted_sol}.
