...
\begin{equation}
    \hat{\Sigma}_\ast = \frac{n-1}{n}\,\sum_i \hat{Y}_{\ast i}\,\hat{Y}_{\ast i}^\top
    ~,
\end{equation}
where $\hat{\Sigma}_\ast$ is the jackknife estimate of the variance tensor of the uncertainty on the full-data prediction $\hat{Y}_\ast$,
and $\hat{Y}_{\ast i}$ is the estimate of $Y_\ast$ made after \emph{leaving out} the one training point $t_i, y_i$.
The square-root of the trace of this uncertainty variance (that is, what you might call the jackknife estimate of the standard error) is shown in \figurename~\ref{fig:uncertainty}.
...
This, for our purposes, is the ratio of the largest eigenvalue (for non-negative definite matrices) to the smallest \emph{non-zero} eigenvalue.
For rectangular matrices it is the ratio of the largest singular value (that is, what you get out of a singular value decomposition) to the smallest nonzero singular value.
...
Related to this, if you have a choice between formulations that involve an inverse $(X^\top\,X)^{-1}\,X^\top\,Y$ and formulations that involve a pseudo-inverse $X^\dagger\,Y$, you should do the latter, because $X^\top\,X$ has the \emph{square} of the condition number of $X$.
...
\appendix
\section{Mathematical arguments and results}
...
HOGG: Maybe show that lstsq beats solve, which beats inv?
