...
% to-do
% -----
% - Is the GP relationship fully encoded in Bishop? If so, cite; if not, get more precise about what's new here. Also, maybe add a footnote in which we give the translation between our notation and Bishop's notation. Oh and thank Espinoza.
% - Should we mention the "dual" representation? HOGG thinks we are going to the dual when we go from XTX to XXT.
...
% - To be submitted to EPJ Data Science https://epjdatascience.springeropen.com/about
...
% text definitions
\newcommand{\documentname}{\textsl{Review}}
...
This \documentname\ delivers a theoretical and practical discussion of linear regression and function interpolation; it does not deliver new research results.
Our only novel contribution is to make a specific connection between regularized linear regression and Gaussian processes.
...
However, if the basis is Fourier, as we chose above in \eqref{eq:basis}, and the spacing between modes ($\Delta\omega =\omega_{j+2}-\omega_j$) is small enough, the kernel approximates the Fourier transform of the square of the weighting function $f(\omega)$ we use to weight the features.
...
We have encouraged you to consider using very big models, but being intentional about regularization.
...
This section completes requirements for submission to \textit{EPJ Data Science}.
...
\paragraph{Funding:}
DWH's work on this project was supported by the Flatiron Institute, a division of the Simons Foundation.
...
The two authors contributed equally to the analysis and writing represented in this work.
