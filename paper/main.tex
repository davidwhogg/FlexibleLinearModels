...
\section{\raggedright Standard linear fitting: Ordinary least squares with a feature embedding}
...
For a concrete example, one common choice is to make the feature embedding functions $g_j()$ terms in a Fourier series
...
Alternative common choices would be to make the embedding fumctions $g_j()$ polynomials or other kinds of ordered basis functions, such as wavelets or spherical harmonics (the latter if, say, the $t_i$ are positions on the sphere).
\begin{figure}[t]
    \begin{mdframed}
    \caption{Examples of basis functions $g_j(t)$ from the basis given in equation~(\ref{eq:basis}). This basis was constructed with length-scale parameter $T=3$. The wavelength in the location space decreases, and the frequency increases, with index $j$.}
    \label{fig:basis}
    \end{mdframed}
\end{figure}
...
\begin{equation}
    ...
    ~.
\end{equation}
This optimization objective (\ref{eq:opt1}) is convex and the optimization problem has a solution in closed form as long as the number of parameters (and features) $p$ is less than the number of training points $n$ (and the matrix $X^\top X$ is invertible):
\begin{equation}
...
This form (\ref{eq:OLS}) of the prediction of new test data is called ordinary least squares (OLS). It has many good properties, some of which are encoded in the Gauss--Markov theorem.
SOLEDAD: PUT WORDS HERE ABOUT WHAT IS PROMISED BY GAUSS--MARKOV. ALSO: REFERENCE?
\begin{figure}[t]
    \begin{mdframed}
...
The prediction (\ref{eq:OLS}) when $p<n$ (the under-paraemeterized or traditional regime) is \emph{affine invariant} in that $p$-dimensional rotations or rescalings of the rectangular $X$ design matrix do not affect predictions.
...
Ridge regression is not affine invariant in the sense that the standard OLS prediction (\ref{eq:OLS}) is; that is, the effect of the regularization depends on the amplitudes and linear combinations of features placed in the feature matrix.
...
\section{\raggedright Over-parameterization}
...
Like (\ref{eq:OLS}), this prediction is \emph{also} called ordinary least squares (OLS), or sometimes ``min-norm least-squares'' to emphasize the degeneracy-breaking choice.
...
SOLEDAD: CHECK WORDING.

The two equations for OLS---(\ref{eq:OLS}) and (\ref{eq:OLS2})---can be unified into one equation (and also generalized to handle non-invertible matrices $X^\top X$ and $X\,X^\top$) if we define the pseudo-inverse $X^\dagger$:
\begin{equation}\label{eq:OLS3}
    \hat{Y}_\ast = X_\ast\,X^\dagger\,Y
    ~,
\end{equation}
where the pseudo-inverse is the matrix made by inverting the singular values and transposing.
That is, if the singular-value decomposition (SVD) of $X$ is $U\,S\,V$, with $S$ diagonal, the SVD of $X^\dagger$ is $V^\top\,S_{\text{nonzero}}^{-1}\,U^\top$, where $S_{\text{nonzero}}^{-1}$ contains the inverses of the non-zero elements of $S$, and zeros where $S$ is zero. SOLEDAD IS THERE A BETTER NOTATION OR EXPLANATION HERE? SHOULD I BREAK THIS OUT INTO EQUATIONS? (SOLEDAD SAYS: Define pseudo-inverse for a diagonal matrix and then use SVD and reduce to the previously stated definition.)
This pseudo-inverse form (\ref{eq:OLS3}) of OLS is extremely general: It works for both the $p\le n$ and $p>n$ cases, and it works when the $X\,X^\top$ or $X^\top\,X$ matrices are not invertible.
...
\section{\raggedright Feature weighting}\label{sec:fwols}
...
\begin{equation}
    ...
    ~.
\end{equation}
This optimization penalizes more strongly the parameters $\beta_j$ corresponding to features $g_j()$ with larger values of $[\Lambda^{1/2}]_{jj}$.
...
\begin{equation}\label{eq:lambdainv}
    ...
    ~.
\end{equation}
We have chosen this form (\ref{eq:f}) for $f(\omega)$ for specific reasons that will become obvious below.
...
\section{\raggedright How to set the number of parameters (and other hyper-parameters)}\label{sec:dd}

There is a lot of literature analyzing the performance of linear regressions as a function of the sizes $n$ and $p$, regularization strengths and forms, and so on (for example, SOLEDAD CITE AND CITE).
...
... HOGG: PLOTS showing cross-validation; the method and the results.
...
\section{\raggedright The Gaussian process: The limit of infinite features}
...
\begin{equation}
    ...
    ~,
\end{equation}
where $k()$ is a non-negative definite definite kernel function.
...
However, if the basis is Fourier, as we chose above in (\ref{eq:basis}), and the spacing between modes ($\Delta\omega =\omega_{j+2}-\omega_j$) is small enough, the kernel becomes very close to the Fourier transform of the square of the weighting function $f(\omega)$ we use to weight the features.

That is, in the case of the specific example of weight function $f(\omega)$ given in equation (\ref{eq:f}), we can connect our feature-weighted OLS to an equivalent GP if we know the fourier transform of the square of $f(\omega)$. We chose that specific form for $f(\omega)$ because it has a square that is a member of a Fourier-transform pair:
\begin{align}
    \FT[F(t)] &= [f(\omega)]^2
    \\ \label{eq:F}
...
\begin{align}
    ...
    ~.
\end{align}
The comparison of the GP with this kernel to the equivalent feature-weighted OLS with large (but finite) number of parameters $p$ is shown in \figurename~\ref{fig:gp}.
\begin{figure}[t]
    \begin{mdframed}
    \caption{Comparison of feature-weighted ordinary least squares with a Gaussian process. The figure shows the OLS fit, the feature-weighted version with the particular feature weighting given in (\ref{eq:f}), and the GP fit using the kernel (\ref{eq:F}) that is the Fourier transform of the square of the feature weighting. The feature-weighted fit and the GP are essentially identical, as expected.}
    \label{fig:gp}
    \end{mdframed}
\end{figure}
...
RANDOM CONJECTURAL SENTENCE; move earlier: Provided that the spacing of the Fourier modes in frequency space is $\Delta\omega\ll 1 / \Delta t_{\max}$ and the maximum frequency $\floor{p / 2}\,\Delta\omega \gg 1 / \min(s, \Delta t_{\min})$, where
\begin{align}
    \Delta t_{\min} = \min_{i\ne j}|t_i - t_j|
    \\
    ...
    ~,
\end{align}
the OLS with feature weights $f(\omega)$ will be very similar to the mean of a GP with kernel $k(\Delta t) = \FT[f^2]$.

At the end of \sectionname~\ref{sec:fwols}, we discussed including not just feature weights in a matrix $\Lambda^{-1}$ but also data weights in a matrix $C^{-1}$. The GP also permits this, and it is often a very good idea. The generalization of equations (\ref{eq:LambdaC1}) and (\ref{eq:LambdaC2}) to the GP case is 
\begin{equation}
    \hat{Y}_\ast = K_\ast\,(K + C)^{-1}\,Y
    ~.
\end{equation}
As before, in contexts where you have independent uncertainties on each element $y_i$ of your training data $Y$, $C^{-1}$ would naturally be set to the diagonal matrix containing the inverses of the variances of those uncertainties.
This form is used a lot in astronomy and physics (HOGG CITE SOME THINGS?) and it is also the standard form given for the Wiener filter (CITE).

\section{\raggedright Interpolators, running means, filters, and so forth}

HOGG: MAYBE THIS BECOMES JUST PART OF THE DISCUSSION?

HOGG: In what sense are all these things the same things? All I got is that if they are linear---meaning that the prediction $\hat{Y}_\ast$ can be written as a linear operator acting on the training data $Y$---then they probably can be written in some form like feature-weighted OLS in the $p>>n$ regime.
...
HOGG: As for (3): Resample the $t_i$ values maybe? As for (4): Brute-force search? It isn't really possible to do (4) without a model for the behavior of a scientist!! Refer back to the subjectivity of it all.
...
Although the examples are toy, the implementation of everything can be generalized for real-data situations.

Things here about inv(), solve(), lstsq(), and so on. Be sure to talk about the GP as well as the OLS cases here.
...
\begin{itemize}
    ...
    \item make a bibliography and cite stuff!
    \item Make sure this appears above in the uncertainties section: What do you do when your locations $t_i$ are themselves noisy or uncertain? Recall our fundamental assumption. There is a literature on this, and it isn't easy. One option: Linearize and add variances. Only works for uncertainties that are small wrt model features. Another option: Infer the $t_i$. That's often extremely hard.
\end{itemize}
